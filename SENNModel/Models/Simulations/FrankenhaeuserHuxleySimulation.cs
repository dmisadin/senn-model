...
public class FrankenhaeuserHuxleySimulation : BaseSimulation, ISimulation
{
    public FrankenhaeuserHuxleySimulation(FileExporter fileExporter) : base(fileExporter)
    {
    }
            ...
            for (int i = 1; i <= jt; i++)
            {
                ...
                switch (state.MembraneModel)
                {
                    ...
                    case MembraneModel.ChiuRitchieRogartStagg:
                        // CRRS initial conditions at rest (-70mV)
                        // Approximate steady-state values for mammalian myelinated axons
                        state.Y[L] = 0.8;       // h(0) - sodium inactivation
                        state.Y[L + 1] = 0.05;  // m(0) - sodium activation
                        state.Y[L + 2] = 0.0;    // unused slot
                        state.Y[L + 3] = 0.3;    // n(0) - potassium activation
                        break;

                    case MembraneModel.McIntyreRichardsonGrill:
                        // MRG initial conditions at rest (-90mV for human peripheral nerve)
                        // Steady-state values for human peripheral nerve fibers
                        state.Y[L] = 0.9;       // h_t(0) - transient sodium inactivation
                        state.Y[L + 1] = 0.05;  // m_t(0) - transient sodium activation
                        state.Y[L + 2] = 0.05;  // m_p(0) - persistent sodium activation
                        state.Y[L + 3] = 0.1;   // s(0) - slow potassium activation
                        break;
    ...
    private bool RunSingleIterationAndMaybeUpdateUIO(SennState state)
    {
        ...
        // Write sentinel "0 0" to mark start of new iteration
        state.Out17?.WriteLine("0 0");
        state.Out30?.WriteLine("0 0");
    ...
    protected override void OutputStep(double x, SennState s, int iHalf, int nDim)
    {
